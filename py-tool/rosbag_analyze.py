...
class CameraSubscriber(Node):
    ...
    def image_callback_factory(self, camera_name, ir_side):
        def callback(msg):
            ...
            if topic_name not in self.first_frame_timestamp:
                ...
                self.get_logger().info(f"First frame timestamp for {topic_name}: {self.first_frame_timestamp[topic_name]}")
            ...
            if topic_name in self.previous_timestamp:
                ...
                if time_diff > self.default_diff:
                    self.get_logger().info(f"Time difference for {topic_name} between current and previous frame: {time_diff:.6f} seconds")
                    self.get_logger().info(f"Current time: {current_time}, Previous time: {self.previous_timestamp[topic_name]}")
    ...
    def check_missing_data(self):
        ...
        for camera in ['front_camera', 'left_camera', 'rear_camera', 'right_camera']:
            for ir_side in ['left_ir', 'right_ir']:
                topic_name = f'{camera}_{ir_side}'
                if topic_name in self.last_received_time and current_time - self.last_received_time[topic_name] > 2.0:
                    if topic_name in self.last_frame_timestamp:
                        self.get_logger().warning(f"No data received for {topic_name} in the last 2 seconds. "
